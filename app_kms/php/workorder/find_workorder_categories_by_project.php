...
// $projectQuery = $pdo->prepare("SELECT * FROM workorder  WHERE WorkOrderNumber = ?");
// $projectQuery->execute([$workorder]);


$workorder = $_POST["workorder"];

$projectQuery = $pdo->prepare("SELECT *,SUM(qty) as sum_all_qty FROM work_order_categories_items  WHERE workorder = ? GROUP BY wo_category_short_name");
$projectQuery->execute([$workorder]);
