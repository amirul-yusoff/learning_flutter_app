...
class ProjectByEmpoyeeCodePage extends StatefulWidget {
  final User user;
  const ProjectByEmpoyeeCodePage({Key? key, required this.user})
      : super(key: key);

  @override
  State<ProjectByEmpoyeeCodePage> createState() =>
      _ProjectByEmpoyeeCodePageState();
}

class _ProjectByEmpoyeeCodePageState extends State<ProjectByEmpoyeeCodePage> {
  ...
  Future getAllCategory() async {
    ...
    var usercode = 'J055';
    http.post(Uri.parse(MyConfig.server + "/project_code_by_employee_code.php"),
        body: {
          "employeeCode": widget.user.employeeCode,
        }).then((response) {
      print("response.statusCode");
      print(MyConfig.server + "/project_code_by_employee_code.php");
      print(response.statusCode);
  ...
  Future<void> _runFilter(String enteredKeyword) async {
    http.post(Uri.parse(MyConfig.server + "/project_code_by_employee_code.php"),
        body: {
          "employeeCode": widget.user.employeeCode.toString(),
        }).then((response) {
